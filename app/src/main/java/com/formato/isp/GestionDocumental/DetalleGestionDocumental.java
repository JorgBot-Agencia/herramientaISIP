...
public class DetalleGestionDocumental extends Fragment {
    ...
    public static ArrayList<Area> areaspdf2;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (getArguments() != null) {
            ...
            img=getArguments().getString("logo");
    ...
    public void calcularPromediosxrevision(ArrayList<Area> arr) {
        ...
        if(c8!=0) {
            arr.add(new Area(8,0,0,prom8 / c8));
        }
        if(c9!=0) {
    ...
    public void CargarListView(JSONArray ja) throws JSONException {
        ...
        for (int i = 0; i < ja.length(); i++) {
            ...
            e.setLogo(jsonObj.getString("revi_imagen"));
            lista.add(e);
    ...
    public void Cargararray(JSONArray ja) throws JSONException {
        ...
        templatePDF.addparrafo(descripcion);
        templatePDF.addtitulo("RESULTADOS");
        templatePDF.addtitulocentrado("Resultado total");

        if(uri.length()>4) {
            templatePDF.creartablaimagenGRAFICA2(loadedImage,"MIINFORME",uri);

        }else{
            templatePDF.addparrafo("La empresa aun no cuenta con un reporte grafico, debe generarlo al finalizar la encuesta");
        }
        templatePDF.addtitulo("Resultados especificos");
        //templatePDF.creartablapers2(header,getResltEsp(),areaspdf);
        templatePDF.creartablapers3(header,getResltEsp(),areaspdf);
    ...
    public Bitmap downloadFile(String imageHttpAddress) {
        ...
        try {
            ...
            Toast.makeText(root.getContext(), "Error cargando la imagen: "+e.getMessage(), Toast.LENGTH_LONG).show();
            e.printStackTrace();
        }
        return loadedImage;
    }
        ...
        if(lista.size()==0){
            ...
            mAdapter.setOnItemClickListener(new AdapterListBasicDetalleGestion.OnItemClickListener() {
                ...
                public void onItemClick(View view, Revision obj, int position) {
                    uri= obj.getLogo();
                    new GetImagendormURL().execute(uri);
                    Toast.makeText(root.getContext(),"Generando PDF...",Toast.LENGTH_LONG).show();
                    consultar_escalasXrevision(obj.getRevi_id());

                }
            });
        }
        Picasso.with(root.getContext()).load(img).into(foto);
    }

    public class GetImagendormURL extends AsyncTask<String,Void,Bitmap>{
        ImageView im;

        public  GetImagendormURL(){
        }
        ...
        protected Bitmap doInBackground(String... strings) {
            ...
            try {
                ...
                loadedImage= BitmapFactory.decodeStream(srt);
            }catch (Exception e){
                e.printStackTrace();
            }
            return loadedImage;
